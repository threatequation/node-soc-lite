var rulesArray = [
	...
	{
		"id": "16",
		"rule": "([^*\\s\\w,.\\\/?+-]\\s*)?(?<![a-mo-z]\\s)(?<![a-z\\\/_@])(\\s*return\\s*)?(?:alert|inputbox|showmod(?:al|eless)dialog|showhelp|infinity|isnan|isnull|iterator|msgbox|executeglobal|expression|prompt|write(?:ln)?|confirm|dialog|urn|(?:un)?eval|exec|execscript|tostring|status|execute|window|unescape|navigate|jquery|getscript|extend|prototype)",
	...
	{
		"id": "1000",
		"rule": "\\x0A\\b(?:(?:reply-)?to|b?cc|content-[td]\\w)\\b\\s*:.*?\\@",
		"description": "Detects common mail header injections",
		"tags": {
			"tag": [
				"mh"
	...
	},
	{
		"id": "1001",
		"rule": "^[^?]*\\.(?:bat|cmd)(?:\\W|$)",
		"description": "Detects Potential reflected file download attempt",
		"tags": {
			"tag": [
				"lfi"
			]
