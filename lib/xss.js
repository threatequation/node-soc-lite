...
module.exports = function (req, res) {
    ...
    loggGenerator(req, 'la');
    if (req.method === "GET") {
        for (var index in req.query) {
            var detect = ruleChecker.xssFilter(decodeURI(req.query[index]));
            if (detect) {
                // var sanitizedValue = sanitizeHtml(req.query[index]);
                // sanitize query string
                req.query[index] = purifier.xssSanitizer(req.query[index]);
            }
        }
    ...
    if (req.method === "POST"){
        for (var index in req.body) {
            var detect = ruleChecker.xssFilter(decodeURI(req.body[index]));
            if (detect) {
                // var sanitizedValue = sanitizeHtml(req.query[index]);
                // sanitize query string
                console.log(purifier.xssSanitizer(req.body[index]));
                req.body[index] = purifier.xssSanitizer(req.body[index]);
            }
        }
    }
